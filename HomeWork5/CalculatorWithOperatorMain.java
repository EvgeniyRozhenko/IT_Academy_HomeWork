package HomeWork5;
/*Изучаем: return, инкапсуляция, наследование, переоределение, интефрейсы, циклы, Math, операторы ветвления, декораторы.
Калькулятор:
Суть задания:
	Написать различные варианты калькуляторов которые делают одно и тоже различными способами (какой-то использует только
	собственные методы, какой-то обращается к библиотеке Math). Тем самым вы в полной мере испробуете инкапсуляцию, наследование.
...
2. Создать класс CalculatorWithOperator.
	...
	2.2 В классе должны присутствовать математические методы:
		2.2.1 4 базовых математических метода (деления, умножение, вычитание, сложение) каждый из этих методов должен
		принимать два параметра (определитесь с их типами) и должны возвращать результать (определиться с возвращаемым
		типом результата) при помощи ключевого слово return.
		2.2.2 3 метода (Возведение в степень дробного положительного числа в целую степень, Модуль числа, Корень из числа
		(если у Вас плохо с математикой можно в этом методе воспользоваться Math)).
...
5*. Создать класс CalculatorWithCounter.
	...
	5.5 В классе должен быть метод long getCountOperation() который должен возвращать количество использований данного калькулятора.
	5.6 Создать класс CalculatorWithCounterMain в котором будет точка входа (main метод). В main методе требуется создать
	экземпляр калькулятора и используя методы из данного экземпляра посчитать выражения из задания 1. Вывести в консоль результат.
6*. Создать интерфейс ICalculator.
	6.1 В данном интерфейсе должны быть объявлены методы:
		...
		6.1.2 3 метода (Возведение в степень дробного положительного числа в целую степень, Модуль числа, Корень из числа).
	6.2 Данный интерфейс должен быть прописан во всех классах наших калькуляторов кроме CalculatorWithCounter (implements ICalculator)
	6.3 Создать класс CalculatorWithCounterInterface. Данный класс это полная копия класса CalculatorWithCounter за
	исключением конструкторов. Тут будет один коструктор принимающий объекты интерфейса ICalculator
7*. Создать CalculatorWithMemory.
	7.1 Данный калькулятор предназначен для того чтобы расширить возможности калькулятора и обеспечить его дополнительной
	...
	7.5 Функция памяти работает через методы:
		...
		7.5.2 Получить из памяти записанное значение. При получении записи из памяти память стирается, при записи нового
		значения память перезаписывается.
	7.6 Создать класс CalculatorWithMemoryMain в котором будет точка входа (main метод). В main методе требуется создать
	экземпляр калькулятора и используя методы из данного экземпляра посчитать выражения из задания 1. Вывести в консоль
	результат. В мэйне запрещается использование переменных для хранения значений участвующих в просчёте, а также результатов
	работы методов калькулятора.
8*. Создать класс CalculatorWithCounterDecorator и CalculatorWithMemoryDecorator
	8.1 Отличие от предыдущих CalculatorWithCounter и CalculatorWithMemory в том что данные классы реализуют интерфейс ICalculator.
	8.2 Данный подход позволяет назвать данный классы "Декораторами". Декоратор - это популярный паттерн который позволяет
	комбинирова и дополнять новыми возможностями объекты (в нашем случае калькуляторы) не меняя код в их классе. Тем
	самым мы приходим к тому что мы не наследуем классы для их расширения и не привязываемся к реализации конкретного калькулятора.
	...
	8.4 Добавить в эти классы метод ICalculator getCalculator() который будут возвращать вложенный в них калькулятор.
	8.5 Создать класс CalculatorDecoratorMain в котором будет точка входа (main метод). Cоздать переменную типа ICalculator
	записав в неё новый экземпляр класса CalculatorWithCounterDecorator которому в конструктор был передан новый экземпляр
	...
	8.6 Вывести в консоль результат. Вывести количество использований калькулятора. Вывести последнее сохранённое значение в памяти.
9**. Написать класс CalculatorStringExpression. Паттерн Адаптер
	...
	9.6 Класс должен контролировать что в строке могут быть только допустие символы (числа, операторы, скобки, константы)
...
public class CalculatorWithOperatorMain{
    public static void main(String[] args) {
        ...
        CalculatorWithOperator value2 = new CalculatorWithOperator();
        CalculatorWithOperator value3 = new CalculatorWithOperator();
        CalculatorWithOperator value4 = new CalculatorWithOperator();
        CalculatorWithOperator sum = new CalculatorWithOperator();
//        System.out.println("value1: " + value1.division(28, 5));
//        System.out.println("value2: " + value2.power(value1.division(28, 5), 2));
//        System.out.println("value3: " + value3.multiplication(15, 7));
//        System.out.println("value4: " + value4.addition(4.1, value3.multiplication(15, 7)));
